...
async function getItems(location, recipients) {
  ...
  if (!fs.existsSync(`./lastItems/${location}.json`)) {
    fs.writeFileSync(`./lastItems/${location}.json`, JSON.stringify({ [item.item.item_id]: item.items_available }, null, 2));
  } else {
    lastItems = await JSON.parse(fs.readFileSync(`./lastItems/${location}.json`));
    lastItems[location] = item.items_available;
    fs.writeFileSync(`./lastItems/${location}.json`, JSON.stringify(lastItems, null, 2));
  }
