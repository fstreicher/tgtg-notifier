...
async function getItems() {
  ...
  let lastItems = 0;
  if (!fs.existsSync('./lastItems.txt')) {
    fs.writeFileSync('./lastItems.txt', JSON.stringify({ [item.item.item_id]: item.items_available }));
  } else {
    lastItems = await JSON.parse(fs.readFileSync('./lastItems.txt'));
