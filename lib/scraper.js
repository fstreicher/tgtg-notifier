...
async function getItems(location, recipients) {
  const res = await getItem(location);

  if (res.status !== 200) {
    if (!fs.existsSync('./reqFailed.txt')) {
      fs.writeFileSync('./reqFailed.txt');
      transporter.sendMail({
        from: `"TGTG Notifier" <${process.env.NODEMAILER_SENDER}>`,
        to: `"${process.env.NODEMAILER_SENDER_NAME}" <${process.env.NODEMAILER_SENDER}>`,
        subject: `TGTG Request failed: ${res.status}`,
        text: `TGTG Request failed: ${res.status}`,
        html: `TGTG Request failed: <b>${res.status}</b>`
      });
    } else {
      return;
    }
  ...
  if (item.items_available > 0) {
    if (!(item.items_available == lastItems[location])) {
      ...
      recipients.forEach(recipient => {
        ...
        if (recipient.trigger === 'available' && lastItems !== 0) {
